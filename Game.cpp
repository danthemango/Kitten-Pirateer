...
void Game::key(unsigned char key, int x, int y)
{
    switch (key)
    {
        //Keys to call control functions here.
    }
    //glutPostRedisplay();
}

//Commented out as the spashscreen has been made into a Menu object.
...
void Game::update()
{
   ...
   if(!Game::c_running)
   {
		...
		return Menu::spashscreen();
	 
   }

	//Call the update function of the GameObjects here.
...
void Game::init() {
    ...
    glutCreateWindow("Kitty Piratier: Zombie Island");
   ...
   m_backgroundTexture= ImageLoader::LoadTexture( "" );
